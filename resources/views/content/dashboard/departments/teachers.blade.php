...
@section('title', __('Department Settings'))
    ...
    .nav-tabs .nav-link.active {
        ...
        <div class="card p-4">
            <ul class="nav nav-tabs mb-4">
                <li class="nav-item"><a class="nav-link" href="{{ route('department.get', $department->id) }}"><span class="mdi mdi-cog-outline me-2"></span>{{ __('Edit Department') }}</a></li>
                <li class="nav-item"><a class="nav-link active" href="javascript:void(0);"><span class="mdi mdi-key-outline me-2"></span>{{ __('Teachers') }}</a></li>
            ...
            <div class="row {{ app()->getLocale() == "ar" ? "me-1" : "ms-1" }} mb-2">
                ...
                <button class="btn btn-icon btn-outline-primary m-1" id="" data-bs-toggle="modal" data-bs-target="#createDepartmentModal"><span class="mdi mdi-plus-outline"></span></button>
        ...
        <script type="text/javascript">
            ...
            function viewTeacher(id) {
                window.location.href = "{{ route('teacher.get', '') }}/" + id;
            }
    
            function showContextMenu(id, x, y) {
    
                var contextMenu = $('<ul class="context-menu" dir="{{ app()->isLocale("ar") ? "rtl" : "" }}"></ul>')
                    .append('<li><a onclick="viewTeacher(' + id + ')"><i class="tf-icons mdi mdi-arrow-right {{ app()->isLocale("ar") ? "ms-1" : "me-1" }}"></i>{{ __("Edit") }}</a></li>')
        ...
        $(document).ready(function() {
                table = $('#table').DataTable({
                    ...
                    ajax: {
                        url: "{{ route('departments.teachers', ['id' => $department->id]) }}",
                        data: function(d) {
                    ...
                    rowCallback: function(row, data) {
                        $(row).attr('id', 'department_' + data.id);
    
                        // $(row).on('dblclick', function() {
                        //     window.location.href = "{{ url('department') }}/" + data.id;
                        // });
                        ...
                        var departmentId = parseInt($checkbox.val());
                ...
                $('#createDepartmentForm').submit(function(event) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            $('#createDepartmentForm .form-control').removeClass('valid');
                            $('#createDepartmentForm .form-select').removeClass('valid');
                            table.ajax.reload();
                        },
                        error: function(xhr, textStatus, errorThrown) {
                            $('#loading').hide();
