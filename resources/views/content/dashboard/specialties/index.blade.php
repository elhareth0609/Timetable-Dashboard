...
<style>
        ...
        <div class="card p-4">
            <ul class="nav nav-tabs mb-4">
                <li class="nav-item"><a class="nav-link active" href="javascript:void(0);"><span class="mdi mdi-cog-outline me-2"></span>{{ __('Edit Specialty') }}</a></li>
            ...
            <form id="editProfileForm" action="{{ route('settings.account.update') }}" method="POST">
                ...
                <div class="row mb-4">
                    ...
                    <div class="form-group form-group-floating col-md-6 {{ app()->getLocale() == "ar" ? "input-rtl" : "" }} mb-3">
                        ...
                        <input type="text" class="form-control" id="name" name="name" value="{{ $specialty->name }}" placeholder="{{ __('Name') }}" >
                    </div>

                    <div class="form-group form-group-floating col-md-6 {{ app()->getLocale() == "ar" ? "input-rtl" : "" }} mb-3">
                        <label for="code" class="form-label">{{ __('Code') }}</label>
                        <input type="text" class="form-control" id="code" name="code" value="{{ $specialty->code }}" placeholder="{{ __('Code') }}" >
                    </div>

                    <div class="form-group form-group-floating col-md-6 {{ app()->getLocale() == "ar" ? "input-rtl" : "" }} mb-3">
                        <label for="name_ar" class="form-label">{{ __('Name (Arabic)') }}</label>
                        <input type="text" class="form-control" id="name_ar" name="name_ar" value="{{ $specialty->name_ar }}" placeholder="{{ __('Name (Arabic)') }}" >
                    </div>
