    ...
    .nav-tabs .nav-link.active {
        ...
        <div class="card p-4">
            <ul class="nav nav-tabs mb-4">
                ...
                <li class="nav-item"><a class="nav-link active" href="javascript:void(0);"><span class="mdi mdi-key-outline me-2"></span>{{ __('Groups') }}</a></li>
        ...
        <script type="text/javascript">
            ...
            //         $('#edit_code').val(group.code);
            //         $('#edit_name').val(group.name);
            //         $('#edit_name_ar').val(group.name_ar);
            //         $('#edit_email').val(group.email);
            ...
            function viewGroup(id) {
                window.location.href = "{{ route('group.get', '') }}/" + id;
            }
    
            function showContextMenu(id, x, y) {
    
                var contextMenu = $('<ul class="context-menu" dir="{{ app()->isLocale("ar") ? "rtl" : "" }}"></ul>')
                    .append('<li><a onclick="viewGroup(' + id + ')"><i class="tf-icons mdi mdi-arrow-right {{ app()->isLocale("ar") ? "ms-1" : "me-1" }}"></i>{{ __("Edit") }}</a></li>')
                    .append('<li class="px-0 pe-none"><div class="divider border-top my-0"></div></li>')
                    .append('<li><a onclick="deleteGroup(' + id + ')"><i class="tf-icons mdi mdi-trash-can-outline {{ app()->isLocale("ar") ? "ms-1" : "me-1" }}"></i>{{ __("Delete") }}</a></li>');
        ...
        $(document).ready(function() {
                table = $('#table').DataTable({
                    ...
                    ajax: {
                        url: "{{ route('specialty.groups', ['id' => $specialty->id]) }}",
                        data: function(d) {
                    ...
                    rowCallback: function(row, data) {
                        ...
                        var groupId = parseInt($checkbox.val());
                ...
                $('#editFacultyForm').submit(function(event) {
                    ...
                    $.ajax({
                        url: "{{ route('group.update', ':id') }}".replace(':id', id),
                        type: $(this).attr('method'),
                        data: formData,
                        dataType: 'json',
                        success: function(response) {
