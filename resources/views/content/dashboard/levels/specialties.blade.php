    ...
    .nav-tabs .nav-link.active {
        ...
        <div class="card p-4">
            <ul class="nav nav-tabs mb-4">
                <li class="nav-item"><a class="nav-link" href="{{ route('level.get', $level->id) }}"><span class="mdi mdi-cog-outline me-2"></span>{{ __('Edit Faculty') }}</a></li>
                <li class="nav-item"><a class="nav-link active" href="javascript:void(0);"><span class="mdi mdi-key-outline me-2"></span>{{ __('Specialties') }}</a></li>
            </ul>
        ...
        <script type="text/javascript">
            ...
            //         $('#edit_code').val(specialty.code);
            //         $('#edit_name').val(specialty.name);
            //         $('#edit_name_ar').val(specialty.name_ar);
            //         $('#edit_email').val(specialty.email);
            //         $('#edit_description').val(specialty.description);
            ...
            function viewSpecialty(id) {
                window.location.href = "{{ route('specialty.get', '') }}/" + id;
            }
        ...
        $(document).ready(function() {
                table = $('#table').DataTable({
                    ...
                    ajax: {
                        url: "{{ route('level.specialties', ['id' => $level->id]) }}",
                        data: function(d) {
                    ...
                    columns: [
                        ...
                        {data: 'department_id', name: '{{__("Department")}}'},
                    ...
                    rowCallback: function(row, data) {
                        $(row).attr('id', 'specialty_' + data.id);
    
                        // $(row).on('dblclick', function() {
                        //     window.location.href = "{{ url('specialty') }}/" + data.id;
                        // });
                        ...
                        var specialtyId = parseInt($checkbox.val());
                ...
                $('#editFacultyForm').submit(function(event) {
                    ...
                    $.ajax({
                        url: "{{ route('specialty.update', ':id') }}".replace(':id', id),
                        type: $(this).attr('method'),
                        data: formData,
                        dataType: 'json',
                        success: function(response) {
